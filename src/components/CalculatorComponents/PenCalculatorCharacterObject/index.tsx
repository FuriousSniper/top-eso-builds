                ...
                <Collapsible open={true}>
                    <>
                        {props.char.class === "Arcanist" &&
                            <GenericInput type={"number"} name={"Arcanist Passive"} value={arcanistPassive} id={`arcanistPassive${props.char.id}`} onChange={(event) => setArcanistPassive(boundsMinMax(Number(event.target.value),0,6))} min={0} max={6} title={"Increase your Physical and Spell Penetration by 991 per Herald of the Tome ability slotted (e.g. beam, flail, eye)."}/>
                        }
                        {props.char.class === "Necromancer" &&
                            <GenericInput type={"checkbox"} name={"Necromancer Passive"} checked={necroPassive} id={`necroPassive${props.char.id}`} onChange={() => setNecroPassive(!necroPassive)} title={"While a Grave Lord ability is active, your Spell and Physical Penetration are increased by 1500."}/>
                        }
                        {props.char.class === "Nightblade" &&
                            <GenericInput type={"checkbox"} name={"Nightblade Passive"} checked={nbPassive} id={`nbPassive${props.char.id}`} onChange={() => setNbPassive(!nbPassive)} title={"Increases your Physical and Spell Penetration against enemies you are flanking by 2974."}/>
                        }
                        <GenericInput type={"checkbox"} name={"Velothi Mythic"} checked={velothi} id={`velothi${props.char.id}`} onChange={() => setVelothi(!velothi)} title={"Adds 1650 Offensive Penetration, 15% dmg done, Minor Force"}/>
                        <GenericInput type={"number"} name={"Light Armor"} value={lightArmor} id={`lightArmor${props.char.id}`} onChange={(event) => setLightArmor(boundsMinMax(Number(event.target.value),0,7))} min={0} max={7} title={"Increases your Physical and Spell Penetration by 939 for each piece of Light Armor worn."}/>
                        <GenericInput type={"number"} name={"Weapon Mace/Maul"} value={weaponMace} id={`weaponMace${props.char.id}`} onChange={(event) => setWeaponMace(boundsMinMax(Number(event.target.value),0,2))} min={0} max={2} title={"Amount of Mace weapons. Set 0 if you have none, set 2 when using 2h weapon."}/>
                        <GenericInput type={"number"} name={"Sharpened trait(s)"} value={weaponSharp} id={`weaponSharp${props.char.id}`} onChange={(event) => setWeaponSharp(boundsMinMax(Number(event.target.value),0,2))} min={0} max={2} title={"Amount of Sharpened traits on weapons. Set 0 if you have none, set 2 when using 2h weapon"}/>
                        <GenericInput type={"number"} name={"Force of Nature CP"} value={FoN} id={`FoN${props.char.id}`} onChange={(event) => setFoN(boundsMinMax(Number(event.target.value),0,8))}  min={0} max={8} title={"Increases your Offensive Penetration by 660 for every status effect your target has."}/>
                        <GenericInput type={"checkbox"} name={"Piercing CP"} checked={piercing} id={`piercing${props.char.id}`} onChange={() => setPiercing(!piercing)} title={"Grants 350 Offensive Penetration per stage (2 stages = 700 total)."}/>
                        <GenericInput type={"checkbox"} name={"Wood Elf Race"} checked={woodElfRace} id={`woodElfRace${props.char.id}`} onChange={() => setWoodElfRace(!woodElfRace)} title={"Increases your Stealth Detection radius by 3 meters. Increases your Movement Speed by 5% and your Physical and Spell Penetration by 950."}/>
