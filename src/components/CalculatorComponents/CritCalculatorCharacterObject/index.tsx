        ...
        <GenericDisplayField legendText={charObject.name} legendIcon={determineIcon(charObject.class)} parentClassName="charAutoMinHeight">
            <>
                ...
                <Collapsible open={true}>
                    <>
                        <GenericInput type={"number"} name={"Warden Passive"} value={charObject.wardenPassive} id={`wardenPassive-${charObject.id}`} onChange={(event) => handleUpdate(event)} min={0} max={6} title={"Increases your Critical Damage by 5% for each Animal Companion ability slotted. (Animal Companions)"} />
                        <GenericInput type={"checkbox"} name={"Templar Passive"} checked={charObject.templarPassive} id={`templarPassive-${charObject.id}`} onChange={(event) => handleUpdate(event)} title={"Increases your Critical Damage by 10%. (Aedric Spear)"} />
                        <GenericInput type={"checkbox"} name={"Nightblade Passive"} checked={charObject.nbPassive} id={`nbPassive-${charObject.id}`} onChange={(event) => handleUpdate(event)} title={"Increases your Critical Damage by 10%. (Assassination)"} />
                        <GenericInput type={"checkbox"} name={"Arcanist Passive"} checked={charObject.arcanistPassive} id={`arcanistPassive-${charObject.id}`} onChange={(event) => handleUpdate(event)} title={"Warp fate when you generate or consume Crux, increasing your Critical Damage and Critical Healing by 12% for 7 seconds. (Herald of the Tome)"} />
