                ...
                <Collapsible open={true}>
                    <>
                        {props.char.class === "Warden" &&
                            <GenericInput type={"number"} name={"Warden Passive"} value={wardenPassive} id={"wardenPassive"} onChange={(event) => setWardenPassive(boundsMinMax(Number(event.target.value), 0, 6))} min={0} max={6} title={"Increases your Critical Damage by 4% for each Animal Companion ability slotted."} />
                        }
                        {props.char.class === "Templar" &&
                            <GenericInput type={"checkbox"} name={"Templar Passive"} checked={templarPassive} id={"templarPassive"} onChange={() => setTemplarPassive(!templarPassive)} title={"Increases your Critical Damage by 10%."} />
                        }
                        ...
                        <GenericInput type={"number"} name={"Weapon Axe"} value={weaponAxe} id={"weaponAxe"} onChange={(event) => setWeaponAxe(boundsMinMax(Number(event.target.value), 0, 2))} min={0} max={2} title={"Each axe increases your Critical Damage done by 6%. Set 2 if using 2h axe."} />
                        <GenericInput type={"checkbox"} name={"Harpooner mythic"} checked={harpooner} id={"harpooner"} onChange={() => setHarpooner(!harpooner)} title={"Dealing direct damage grants you a stack of Hunter's Focus for 20 seconds, up to 10 stacks max. You can only gain 1 stack of Hunter's Focus per second. Each stack of Hunter's Focus increases your Critical Chance by 110 and your Critical Damage by 1%. Taking direct damage removes 5 stacks of Hunter's Focus, up to once per second. Removing Harpooner's Wading Kilt removes all stacks of Hunter's Focus."} />
                        <GenericInput type={"checkbox"} name={"Backstabber Cp"} checked={backstabberCp} id={"backstabberCp"} onChange={() => setBackstabberCp(!backstabberCp)} title={"Increases your Critical Damage done by 2% per stage against enemies you are flanking (5 stages = 10%)."} />
                        <GenericInput type={"checkbox"} name={"Fighting Finesse Cp"} checked={fFinesseCp} id={"fFinesseCp"} onChange={() => setFFinesseCp(!fFinesseCp)} title={"Increases your Critical Damage and Critical Healing done by 4% per stage (2 stages = 8%)."} />
                        <GenericInput type={"checkbox"} name={"Khajiit Race"} checked={khajiitRace} id={"khajiitRace"} onChange={() => setKhajiitRace(!khajiitRace)} title={"Increases your Critical Damage and Critical Healing by 12%. Decreases your detection radius in Stealth by 3 meters."} />
                        <GenericInput type={"checkbox"} name={"Shadow Mundus"} checked={shadowMundus} id={"shadowMundus"} onChange={() => setShadowMundus(!shadowMundus)} title={"Increases Critical Damage and Healing by 17% (7 divines)"} />
                        <GenericInput type={"checkbox"} name={"Order Wrath set"} checked={orderWrath} id={"orderWrath"} onChange={() => setOrderWrath(!orderWrath)} title={"Adds 943 Critical Chance, Increases your Critical Damage and Critical Healing by 8% (set)."} />
                        <GenericInput type={"number"} name={"Other sources"} value={otherSources} id={"otherSources"} onChange={(event) => setOtherSources(boundsMinMax(Number(event.target.value), 0, 125))} min={0} max={125} title={"Input other unique sources here."} />
                    </>
                </Collapsible>
